...
# Pi's instruction files, AGENTS.override.md, which Codex and Pi load in place
# of AGENTS.md, and Claude Code's CLAUDE.local.md. Each in both forms the
# patterns have to catch: the relative name the Pi extension passes from the
# vault root, and an absolute path from a hook. Two look-alikes, also in both
# forms, are the negative control, because a scan widened to every file under
# .pi/ would pass the positive half as well.
...
for pi_f in .pi/SYSTEM.md .pi/APPEND_SYSTEM.md .pi/skills/probe/SKILL.md .pi/prompts/probe.md \
            AGENTS.override.md sub/.pi/SYSTEM.md sub/AGENTS.override.md CLAUDE.local.md sub/CLAUDE.local.md; do
  printf 'steering file with a hidden character:\342\200\213\n' > "$WORK/$pi_f"
...
for pi_f in .pi/other.md pi/SYSTEM.md; do
  ...
  pi_rel=$(cd "$WORK" && CLAUDE_PROJECT_DIR="$WORK" bash "$HOOK" -- "$pi_f" 2>&1 | strip_notices)
  pi_abs=$(lint "$WORK/$pi_f" | strip_notices)
  printf '%s' "$pi_rel" | grep -q "U+200B" && pi_steer_missed="$pi_steer_missed widened-to:relative:$pi_f"
  printf '%s' "$pi_abs" | grep -q "U+200B" && pi_steer_missed="$pi_steer_missed widened-to:absolute:$pi_f"
done
if [ -z "$pi_steer_missed" ]; then
  ok "invisible-char scan covers Pi's .pi/SYSTEM.md, .pi/APPEND_SYSTEM.md, .pi/skills/, .pi/prompts/, AGENTS.override.md and CLAUDE.local.md, by relative and absolute path, and not .pi/other.md or pi/SYSTEM.md by either"
else
  bad "Pi steering files, AGENTS.override.md and CLAUDE.local.md scan --$pi_steer_missed"
fi
rm -rf "$WORK/.pi" "$WORK/sub" "$WORK/pi" "$WORK/AGENTS.override.md" "$WORK/CLAUDE.local.md"
...
# Pi's approval surface is project trust, and an extension loaded with -e may
# answer Pi's project_trust event. The adapter must never do that, or loading
# it would trust every project the session opens.
pi_trust_hits() {  # pi_trust_hits <file>... - prints lines that touch Pi's project trust
  grep -nE 'project_trust|defaultProjectTrust|trust\.json' "$@" 2>/dev/null
}
printf 'pi.on("project_trust", () => ({ trusted: "yes" }))\n' > "$TMP/fake-trust.js"
if [ -n "$(pi_trust_hits "$TMP/fake-trust.js")" ]; then
  ok "positive control: an extension answering Pi's project_trust is caught"
else
  bad "positive control: a project_trust handler went unnoticed"
fi
hits=$(pi_trust_hits "$ROOT/.claude/adapters/pi/vault.js")
if [ -z "$hits" ]; then ok "the Pi extension never answers Pi's project trust"
...
if [ "$pi_node_major" -lt 18 ]; then
  ...
  skip pi-extension-dirlink "Pi extension: needs node 18 or later, and found ${pi_node_version:-no node}"
  skip pi-extension-symlink "Pi extension: needs node 18 or later, and found ${pi_node_version:-no node}"
else
  # Two fake vaults, the second inside a folder named secrets, and one copy of
  # the adapter that sits in no vault at all. The paths are left as mktemp
  # spelled them. On macOS that is /var/..., while Node names the adapter by
  # its real path under /private/var, which is a mismatch Pi can meet as well.
  PIV="$TMP/pi/my vault"
...
#!/usr/bin/env bash
printf '%s\n' "${CLAUDE_PROJECT_DIR:-}" > stub-project-dir
cat > stub-input
PI_STUB_EOF
  done
  mkdir -p "$PIL"
...
const sameDir = (a, b) => {
  const norm = (p) => (win ? resolve(p.trim()).toLowerCase() : resolve(p.trim()))
  return norm(a) === norm(b)
}

async function load(file, cwd) {
  const handlers = {}
  ...
  const at = (dir) => ({
    ...
    sessionManager: { getSessionId: () => "probe-session", getSessionFile: () => join(dir, "probe-session.jsonl") },
  })
  return { handlers, ctx: at(cwd), at, notes }
}
...
// The fixed reasons are compared whole. A call the guard cannot decide is
// answered "failed (<what went wrong>...", and such a case names the cause it
// expects as "failed:<part of it>".
const FAILED = "vault: the secrets guard failed on this call ("
function classify(result) {
  ...
  if (reason === "vault: the secrets guard found no path in this call, so it is refused. Pi's tool input may have changed shape: see docs/harnesses/pi.md") return "no-path"
  if (reason === "vault: the secrets guard cannot read this grep's glob, so it is refused. Pi's tool input may have changed shape: see docs/harnesses/pi.md") return "no-glob"
  if (reason.startsWith(FAILED)) return `failed (${reason.slice(FAILED.length)}`
  return `refused for another reason (${reason})`
}
const WANTED = { "no-path": "refused as having no path", "no-glob": "refused as a glob that is not text" }
const wanted = (want) => WANTED[want] ?? (want.startsWith("failed:") ? `refused as a call the guard cannot decide (${want.slice(7)})` : want)
const answers = (got, want) => (want.startsWith("failed:") ? got.startsWith("failed (") && got.includes(want.slice(7)) : got === want)
...
async function expect(pi, tool, input, want, label, ctx) {
  ...
  verdict(answers(got, want), `Pi guard: ${label} is ${wanted(want)}`, got)
}

const pi = await load(adapterIn(vault), vault)
// The home folder is inside secrets/ for this run, so ~/ has to be expanded
// for the ~ case to be refused.
const cases = [
  ...
  ["read", { path: ".env." }, "refused", ".env., under the .env.* rule"],
  ["read", { path: ".env " }, "refused", ".env with a trailing space"],
  ["read", { path: ".env\u{A0}" }, "refused", ".env with a trailing no-break space, which Pi reads as a space"],
  ...
  ["read", { path: "secrets./x" }, "refused", "secrets./x, a folder name with a trailing dot"],
  ["grep", { pattern: "KEY", path: "secrets." }, "refused", "a grep of secrets. with a trailing dot"],
  ...
  ["read", { path: "file:///x%2Fy" }, "failed:must not include encoded", "a file:// URL Pi cannot open either (the guard fails closed)"],
...
const refusedGlobs = [".env", ".env*", ".ENV*", "*", "{.env,x}", "secret?/**", ".[e]nv", "**/.env", "{.env",
  ...
  ".[a-b-z]nv", "s[a-b-z]crets/x.md", "s?crets[#-.-0]x.md", ".[e]nv ", "*/.e?v\t", ".[e]nv\u{85}"]
...
cases.push(["grep", { pattern: "KEY", glob: "[!a][!b][!c][!d][!e].md" }, "failed:sets that could match a /", "a grep with a glob of five sets that could match a /"])
// Every construct the matcher reads also appears in a glob it lets through, so
// a construct that started to throw would show as a refusal here. a.md\ (an
// escaped trailing space) only shows that such a glob does not throw: without
// the exception for it the glob would still be allowed.
...
const inner = await load(adapterIn(innerVault), innerVault)
await expect(inner, "read", { path: "31-standards/n.md" }, "allowed", "a note of a vault kept inside a folder named secrets")
await expect(inner, "read", { path: "secrets/x" }, "refused", "that vault's own secrets/")
if (win) {
  const shell = `/${innerVault[0].toLowerCase()}${innerVault.slice(2)}/31-standards/n.md`
  await expect(inner, "read", { path: shell }, "allowed", "a Git Bash /c/... path to that vault's note, which Pi converts first")
}

// A copy of the adapter in no vault still guards, measuring from the vault
// Pi was started in, and runs no script at all.
const stray = await load(join(loose, "vault.mjs"), join(vault, "secrets"))
await expect(stray, "read", { path: "k.txt" }, "refused", "a file in secrets/, for a copy of the adapter kept outside any vault")
await expect(stray, "read", { path: "31-standards/n.md" }, "allowed", "a note, for a copy kept outside any vault", stray.at(vault))

writeFileSync(join(vault, ".env"), "PROBE=1\n")
writeFileSync(join(vault, "31-standards", "n.md"), "note\n")
// Folder links are junctions on Windows, which any user may make, so these
// run on every platform. File links need a privilege Windows rarely grants.
const dirType = win ? "junction" : "dir"
let dirLinked = false
try {
  symlinkSync(join(vault, "secrets"), join(vault, "notes-link"), dirType)
  dirLinked = true
} catch (error) {
  say(`skip pi-extension-dirlink Pi guard: this host cannot link a folder (${error.code ?? error.message})`)
}
if (dirLinked) {
  symlinkSync(join(vault, "secrets", "new-folder"), join(vault, "notes-gone"), dirType)
  mkdirSync(join(vault, "sub"), { recursive: true })
  symlinkSync(join(vault, "31-standards"), join(vault, "sub", "secrets"), dirType)
  symlinkSync(join(vault, "loop-b"), join(vault, "loop-a"), dirType)
  symlinkSync(join(vault, "loop-a"), join(vault, "loop-b"), dirType)
  say("ran pi-extension-dirlink")
  await expect(pi, "write", { path: "notes-link/new.md", content: "x" }, "refused", "a new file inside a linked secrets folder")
  ...
  await expect(pi, "read", { path: "loop-a/x" }, "failed:does not end", "a path through a loop of links (the guard fails closed)")
}
...
try {
  ...
  say(`skip pi-extension-symlink Pi guard: this host cannot make a file link (${error.code ?? error.message})`)
}
if (fileLinked) {
...
const result = (p, toolName, input, isError, ctx = p.ctx) =>
  p.handlers.tool_result({ type: "tool_result", toolCallId: "c2", toolName, input, content: [{ type: "text", text: "wrote it" }], isError }, ctx)
const same = (got, want) => JSON.stringify(got) === JSON.stringify(want)
const lintCase = async (p, input, want, label, ctx) => {
  rmSync(lintArgs, { force: true })
  const answer = await result(p, "write", input, false, ctx)
  const got = lintRecord()
  verdict(same(got, want), `Pi lint: ${label}`, JSON.stringify(got))
  return answer
}
...
const answer = await lintCase(pi, { path: "31-standards/p.md", content: "x" }, ["--", "31-standards/p.md"], "a write runs vault-lint.sh -- 31-standards/p.md from the vault root")
const added = answer?.content?.at(-1)?.text ?? ""
verdict(answer?.content?.[0]?.text === "wrote it" && added.includes("probe finding for 31-standards/p.md"),
  "Pi lint: what the lint reports is added to the tool's result, after what the tool said", JSON.stringify(answer))
const lintDir = existsSync(join(vault, "lint-project-dir")) ? readFileSync(join(vault, "lint-project-dir"), "utf8") : ""
verdict(sameDir(lintDir, realpathSync(vault)), "Pi lint: the lint runs with CLAUDE_PROJECT_DIR naming the vault, not the one Pi inherited", lintDir.trim())
rmSync(lintArgs, { force: true })
...
if (dirLinked) {
  ...
  symlinkSync(vault, alias, dirType)
  await lintCase(pi, { path: "31-standards/p.md", content: "x" }, ["--", "31-standards/p.md"], "a session started through a link to the vault lints the vault-relative path", pi.at(alias))
}
rmSync(lintArgs, { force: true })
...
verdict(autoOk, "Pi stub: an automatic compaction sends the session id, trigger auto and the session file", JSON.stringify(stub))
const stubDir = existsSync(join(vault, "stub-project-dir")) ? readFileSync(join(vault, "stub-project-dir"), "utf8") : ""
verdict(sameDir(stubDir, realpathSync(vault)), "Pi stub: the stub runs with CLAUDE_PROJECT_DIR naming the vault", stubDir.trim())
rmSync(stubInput, { force: true })
await compact(pi, "manual")
stub = stubRecord()
verdict(stub?.trigger === "manual", "Pi stub: /compact sends trigger manual", JSON.stringify(stub))
verdict(pi.notes.length === 0, "Pi extension: no warning while both scripts worked", pi.notes.join(" | "))
...
verdict(failed.length === 1, "Pi extension: a lint that exits 3 is reported, once", JSON.stringify(inner.notes))

rmSync(lintArgs, { force: true })
await result(stray, "write", { path: "31-standards/p.md", content: "x" }, false, stray.at(vault))
await result(stray, "write", { path: "31-standards/p.md", content: "x" }, false, stray.at(vault))
const strayNotes = stray.notes.filter((note) => note.includes("no vault holds"))
...
# FAKE_SEE=<vault path> copies that note, as the agent finds it when it starts,
# to FAKE_SEEN=<path>.
[ -n "${FAKE_SEE:-}" ] && [ -n "${FAKE_SEEN:-}" ] && cp -- "$FAKE_SEE" "$FAKE_SEEN" 2>/dev/null
journal() {
...
case "${FAKE_MODE:-nothing}" in
  ...
  promote-bad)    printf 'no frontmatter\n' > "31-standards/rejected-new.md"
                  printf 'overwritten without frontmatter\n' > "${FAKE_OVERWRITE:-31-standards/existing.md}"
...
q r .git" ;;
  ...
  promote-after-leftover) printf -- '---\ntier: long\ntype: standard\n---\n\ngood\n' > "31-standards/good-after-leftover.md"
                  printf 'PROMOTION-SUMMARY: promoted=1 pending=0\n' ;;
  # Create-only modes: each changes a long-tier note that was there before the
  # pass in one way a steered pass could, and ends with a valid summary. The
  # rewrites go through awk and mv, because BSD sed takes no newline in -i.
  promote-supersede) awk 'NR == 1 { print; print "status: superseded"; print "superseded_by: \"[[promoted-successor]]\""; next } { print }' \
                    31-standards/existing.md > 31-standards/existing.md.tmp && mv -f 31-standards/existing.md.tmp 31-standards/existing.md
                  printf 'PROMOTION-SUMMARY: promoted=0 pending=0\n' ;;
  promote-stamp)  awk -v d="$(date +%F)" 'NR == 1 { print; print "last_verified: \"" d "\""; print "confidence: high"; next } { print }' \
                    31-standards/existing.md > 31-standards/existing.md.tmp && mv -f 31-standards/existing.md.tmp 31-standards/existing.md
                  printf 'PROMOTION-SUMMARY: promoted=0 pending=0\n' ;;
  promote-reverse) awk '$0 == "existing" { print "not existing"; next } { print }' \
                    31-standards/existing.md > 31-standards/existing.md.tmp && mv -f 31-standards/existing.md.tmp 31-standards/existing.md
                  printf 'PROMOTION-SUMMARY: promoted=0 pending=0\n' ;;
  promote-wiki-edit) printf 'promoted\n' >> 40-llm-wiki/wiki/existing-entity.md
                  printf 'PROMOTION-SUMMARY: promoted=0 pending=0\n' ;;
  promote-ignored-edit) printf 'pass edit\n' >> 31-standards/ignored-existing.md
                  printf -- '---\ntier: long\ntype: standard\n---\n\nbeside an ignored note\n' > 31-standards/beside-ignored.md
                  printf 'PROMOTION-SUMMARY: promoted=1 pending=0\n' ;;
  promote-syncnew) printf -- '---\ntier: long\ntype: standard\n---\n\nsynced\n' > 31-standards/synced-new.md
                  git add -- 31-standards/synced-new.md >/dev/null 2>&1
                  git -c user.name=sync -c user.email=sync@example.invalid -c commit.gpgsign=false \
                    commit -q --no-verify -m "sync plugin" -- 31-standards/synced-new.md >/dev/null 2>&1
                  printf 'PROMOTION-SUMMARY: promoted=1 pending=0\n' ;;
  promote-append) printf 'promoted again\n' >> "${FAKE_APPEND:?}"
                  printf 'PROMOTION-SUMMARY: promoted=0 pending=0\n' ;;
  promote-glob-new) printf 'no frontmatter\n' > '31-standards/[p]ass-written.md'
                  printf 'PROMOTION-SUMMARY: promoted=1 pending=0\n' ;;
...
if [ "$RV_GIT" -eq 1 ]; then
  ...
  # --- the long tier is create-only ---
  #
  # A pass may add notes to 31-standards/ and 40-llm-wiki/wiki/ but never change
  # one that was there before it started, whoever wrote it. Each control changes
  # a note someone committed in one way a steered pass could, and the runner must
  # refuse the whole pass with its own text and put every note back. Exit 2 is
  # shared with the dirty-note and deleted-note refusals, so each control asserts
  # on that text and on the note it names. Each fixture is checked before its
  ...
  # co_owner_note <path>: true when HEAD holds the note, the work tree matches
  # it, and the commit that last changed it carries no Vault-Pass trailer.
  co_owner_note() {
    local msg
    msg="$(git -C "$RV" log -1 --format='%H%n%B' -- "$1" 2>/dev/null)"
    [ -f "$RV/$1" ] && [ -n "$msg" ] && git -C "$RV" rev-parse -q --verify "HEAD:$1" >/dev/null 2>&1 \
      && tree_matches_head "$RV" "$1" && ! printf '%s\n' "$msg" | grep -q '^Vault-Pass:'
  }
  # co_named <path> [headline]: true when the log entry with that headline, the
  # create-only refusal unless another is named, lists <path> among the indented
  # lines under it, alone or followed by a note in brackets.
  co_named() {
    H="${2:-$CO_TEXT}" P="    $1" awk 'index($0, ENVIRON["H"]) { f = 1; next } f && /^    / { if ($0 == ENVIRON["P"] || index($0, ENVIRON["P"] " (") == 1) { found = 1; exit } next } { f = 0 } END { exit found ? 0 : 1 }' "$PROMO_LOG" 2>/dev/null
  }
  # co_rejected <path>: the copy the put-back kept of <path> in the quarantine.
  co_rejected() {
    find "$CASE_STATE/quarantine" -path "*-rejected/$1" -type f 2>/dev/null | head -n 1
  }
  # co_front <file>: a note's frontmatter lines, without the fences.
  co_front() {
    awk 'NR == 1 { next } /^---/ { exit } { print }' "$1" | tr -d '\r'
  }
  # co_restore <path>: a control that fails by letting the pass commit over its
  # note would fail every later fixture check for the wrong reason, so the note
  # is committed back as it was before the control. After a control that
  # passed there is nothing to put back.
  co_restore() {
    if ! git -C "$RV" diff --quiet "$co_head" HEAD -- "$1" 2>/dev/null; then
      git -C "$RV" checkout -q "$co_head" -- "$1" 2>/dev/null
      git -C "$RV" commit -q --no-verify -m "put the create-only fixture back" -- "$1" >/dev/null 2>&1
    fi
    git -C "$RV" checkout -q -- "$1" 2>/dev/null
  }

  co_case promotion-create-only-edit
  if co_owner_note "$CO_NOTE" && [ ! -e "$RV/31-standards/beside-dirty.md" ]; then
    ran promo-create-only-edit
    expect_rc "promotion-pass: the pass appends to a committed standard and writes a valid new one -> VIOLATION" 2 \
      "$(runner promotion-pass.sh promote-edit)"
    co_copy="$(co_rejected "$CO_NOTE")"
    if co_named "$CO_NOTE" && ! co_named 31-standards/beside-dirty.md \
       && [ "$(git -C "$RV" rev-parse HEAD)" = "$co_head" ] && tree_matches_head "$RV" "$CO_NOTE" \
       && [ -n "$co_copy" ] && grep -qx 'promoted' "$co_copy" \
       && [ ! -e "$RV/31-standards/beside-dirty.md" ] && [ -n "$(co_rejected 31-standards/beside-dirty.md)" ]; then
      ok "create-only: an edit to a committed standard is refused, the note restored and the pass's new note quarantined"
    else
      bad "create-only: an edit to a committed standard was not refused and put back -- log: $(tr '\n' '|' < "$PROMO_LOG" | cut -c1-2000)"
    fi
  else
    bad "create-only edit control: $CO_NOTE is not a clean note committed without a Vault-Pass trailer, so the control did not run"
  fi
  co_restore "$CO_NOTE"
  rm -f "$RV/31-standards/beside-dirty.md"

  co_case promotion-create-only-supersede
  if co_owner_note "$CO_NOTE" && ! grep -q '^status:' "$RV/$CO_NOTE"; then
    ran promo-create-only-supersede
    expect_rc "promotion-pass: the pass marks a committed standard superseded -> VIOLATION" 2 \
      "$(runner promotion-pass.sh promote-supersede)"
    co_copy="$(co_rejected "$CO_NOTE")"
    if co_named "$CO_NOTE" && ! grep -q '^status: superseded' "$RV/$CO_NOTE" && ! grep -q '^superseded_by:' "$RV/$CO_NOTE" \
       && [ "$(git -C "$RV" rev-parse HEAD)" = "$co_head" ] && tree_matches_head "$RV" "$CO_NOTE" \
       && [ -n "$co_copy" ] && grep -q '^status: superseded' "$co_copy"; then
      ok "create-only: a supersession of a committed standard is refused, and the note carries no status: superseded"
    else
      bad "create-only: a supersession of a committed standard was not refused and put back -- log: $(tr '\n' '|' < "$PROMO_LOG" | cut -c1-2000)"
    fi
  else
    bad "create-only supersede control: $CO_NOTE is not a clean committed note without a status, so the control did not run"
  fi
  co_restore "$CO_NOTE"

  co_case promotion-create-only-stamp
  if co_owner_note "$CO_NOTE" && ! grep -Eq '^(last_verified|confidence):' "$RV/$CO_NOTE"; then
    ran promo-create-only-stamp
    expect_rc "promotion-pass: the pass stamps last_verified and confidence on a committed standard without a probe -> VIOLATION" 2 \
      "$(runner promotion-pass.sh promote-stamp)"
    co_copy="$(co_rejected "$CO_NOTE")"
    # The stamped note passes C1-C5, so vault-check alone would have let it in.
    co_stamp_rc=1
    rm -rf "$TMP/co-stamp"
    if [ -n "$co_copy" ] && mkdir -p "$TMP/co-stamp/31-standards" && cp "$co_copy" "$TMP/co-stamp/$CO_NOTE"; then
      CLAUDE_PROJECT_DIR="$TMP/co-stamp" bash "$CHECK" >/dev/null 2>&1
      co_stamp_rc=$?
    fi
    if co_named "$CO_NOTE" && ! grep -q 'CHECK-FAILED' "$PROMO_LOG" && [ "$co_stamp_rc" -eq 0 ] \
       && ! grep -Eq '^(last_verified|confidence):' "$RV/$CO_NOTE" \
       && [ "$(git -C "$RV" rev-parse HEAD)" = "$co_head" ] && tree_matches_head "$RV" "$CO_NOTE" \
       && grep -Eqx 'last_verified: "[0-9]{4}-[0-9]{2}-[0-9]{2}"' "$co_copy" && grep -qx 'confidence: high' "$co_copy"; then
      ok "create-only: an unprobed stamp that passes vault-check is refused by create-only, not by vault-check"
    else
      bad "create-only: an unprobed stamp was not refused as create-only -- vault-check on the stamped copy: $co_stamp_rc, log: $(tr '\n' '|' < "$PROMO_LOG" | cut -c1-2000)"
    fi
  else
    bad "create-only stamp control: $CO_NOTE is not a clean committed note without a stamp, so the control did not run"
  fi
  co_restore "$CO_NOTE"

  co_case promotion-create-only-reverse
  if co_owner_note "$CO_NOTE" && grep -qx 'existing' "$RV/$CO_NOTE" && ! grep -q 'not existing' "$RV/$CO_NOTE"; then
    ran promo-create-only-reverse
    git -C "$RV" show "HEAD:$CO_NOTE" > "$TMP/co-head-note" 2>/dev/null
    expect_rc "promotion-pass: the pass reverses a committed standard's claim and leaves its frontmatter alone -> VIOLATION" 2 \
      "$(runner promotion-pass.sh promote-reverse)"
    co_copy="$(co_rejected "$CO_NOTE")"
    if co_named "$CO_NOTE" && grep -qx 'existing' "$RV/$CO_NOTE" && ! grep -q 'not existing' "$RV/$CO_NOTE" \
       && [ "$(git -C "$RV" rev-parse HEAD)" = "$co_head" ] && tree_matches_head "$RV" "$CO_NOTE" \
       && [ -n "$co_copy" ] && grep -qx 'not existing' "$co_copy" \
       && [ -n "$(co_front "$co_copy")" ] && [ "$(co_front "$co_copy")" = "$(co_front "$TMP/co-head-note")" ]; then
      ok "create-only: a reversed claim under untouched frontmatter is refused and the claim restored"
    else
      bad "create-only: a reversed claim was not refused and put back -- log: $(tr '\n' '|' < "$PROMO_LOG" | cut -c1-2000)"
    fi
  else
    bad "create-only reverse control: $CO_NOTE is not a clean committed note stating its claim, so the control did not run"
  fi
  co_restore "$CO_NOTE"

  # A committed wiki entity, so the wiki half of the long tier is held too.
  printf -- '---\ntier: long\ntype: wiki-entity\n---\n\nan entity\n' > "$RV/$CO_WIKI"
  co_case promotion-create-only-wiki
  if co_owner_note "$CO_WIKI"; then
    ran promo-create-only-wiki
    expect_rc "promotion-pass: the pass appends to a committed wiki entity -> VIOLATION" 2 \
      "$(runner promotion-pass.sh promote-wiki-edit)"
    co_copy="$(co_rejected "$CO_WIKI")"
    if co_named "$CO_WIKI" && [ "$(git -C "$RV" rev-parse HEAD)" = "$co_head" ] && tree_matches_head "$RV" "$CO_WIKI" \
       && [ -n "$co_copy" ] && grep -qx 'promoted' "$co_copy"; then
      ok "create-only: an edit to a committed wiki entity is refused and put back"
    else
      bad "create-only: an edit to a committed wiki entity was not refused and put back -- log: $(tr '\n' '|' < "$PROMO_LOG" | cut -c1-2000)"
    fi
  else
    bad "create-only wiki control: $CO_WIKI is not a clean note committed without a Vault-Pass trailer, so the control did not run"
  fi
  co_restore "$CO_WIKI"

  # A note git ignores is in no commit, so only the snapshot taken before the
  # pass shows it was there. This control is what holds that half of the check.
  cp "$RV/.git/info/exclude" "$TMP/exclude-create-only" 2>/dev/null || : > "$TMP/exclude-create-only"
  printf '31-standards/ignored-existing.md\n' >> "$RV/.git/info/exclude"
  printf -- '---\ntier: long\ntype: standard\n---\n\nignored\n' > "$RV/31-standards/ignored-existing.md"
  co_case promotion-create-only-ignored
  if git -C "$RV" check-ignore -q -- 31-standards/ignored-existing.md \
     && ! git -C "$RV" rev-parse -q --verify "HEAD:31-standards/ignored-existing.md" >/dev/null 2>&1 \
     && [ -z "$(git -C "$RV" status --porcelain --untracked-files=all -- 31-standards/ignored-existing.md)" ] \
     && [ -f "$RV/31-standards/ignored-existing.md" ] && [ ! -e "$RV/31-standards/beside-ignored.md" ]; then
    ran promo-create-only-ignored
    expect_rc "promotion-pass: the pass edits a note git ignores that was there before it, beside a valid new note -> VIOLATION" 2 \
      "$(runner promotion-pass.sh promote-ignored-edit)"
    if co_named 31-standards/ignored-existing.md && ! co_named 31-standards/beside-ignored.md \
       && [ "$(git -C "$RV" rev-parse HEAD)" = "$co_head" ] \
       && grep -qx 'pass edit' "$RV/31-standards/ignored-existing.md" \
       && grep -qF '31-standards/ignored-existing.md (existed before the pass but is in no commit' "$PROMO_LOG" \
       && [ ! -e "$RV/31-standards/beside-ignored.md" ] && [ -n "$(co_rejected 31-standards/beside-ignored.md)" ]; then
      ok "create-only: an edit to a note git ignores that was there before the pass is refused, and the pass's new note quarantined"
    else
      bad "create-only: an edit to a pre-existing ignored note was not refused -- log: $(tr '\n' '|' < "$PROMO_LOG" | cut -c1-2000)"
    fi
  else
    bad "create-only ignored control: 31-standards/ignored-existing.md is not an ignored, uncommitted note on disk, so the control did not run"
  fi
  cp "$TMP/exclude-create-only" "$RV/.git/info/exclude"
  rm -f "$RV/31-standards/ignored-existing.md" "$RV/31-standards/beside-ignored.md"

  co_case promotion-create-only-failpath
  if co_owner_note "$CO_NOTE"; then
    ran promo-create-only-failpath
    expect_rc "promotion-pass: a failing pass that appended to a committed standard -> VIOLATION" 2 \
      "$(runner promotion-pass.sh promote-edit-fail)"
    co_copy="$(co_rejected "$CO_NOTE")"
    if co_named "$CO_NOTE" && grep -q 'promotion-agent exited with code 3' "$PROMO_LOG" \
       && tree_matches_head "$RV" "$CO_NOTE" && [ -n "$co_copy" ] && grep -qx 'promoted' "$co_copy" \
       && ! grep -qF "$CO_NOTE" "$CASE_STATE/promotion-pass.uncommitted" 2>/dev/null; then
      ok "create-only: a failing pass's edit to a committed standard is put back, not left for the next run"
    else
      bad "create-only: a failing pass's edit to a committed standard was left or recorded -- log: $(tr '\n' '|' < "$PROMO_LOG" | cut -c1-2000)"
    fi
  else
    bad "create-only failpath control: $CO_NOTE is not a clean note committed without a Vault-Pass trailer, so the control did not run"
  fi
  co_restore "$CO_NOTE"

  co_case promotion-create-only-timeout
  if co_owner_note "$CO_NOTE" && [ ! -e "$RV/31-standards/beside-hang.md" ]; then
    ran promo-create-only-timeout
    expect_rc "promotion-pass: a pass killed after appending to a committed standard -> VIOLATION" 2 \
      "$(runner promotion-pass.sh promote-edit-hang PROMOTION_PASS_TIMEOUT=2)"
    co_copy="$(co_rejected "$CO_NOTE")"
    if co_named "$CO_NOTE" && grep -q 'TIMEOUT: promotion-agent exceeded' "$PROMO_LOG" \
       && tree_matches_head "$RV" "$CO_NOTE" && [ -n "$co_copy" ] && grep -qx 'promoted' "$co_copy" \
       && [ ! -e "$RV/31-standards/beside-hang.md" ] && [ -n "$(co_rejected 31-standards/beside-hang.md)" ] \
       && ! grep -qF "$CO_NOTE" "$CASE_STATE/promotion-pass.uncommitted" 2>/dev/null; then
      ok "create-only: a timed-out pass's edit to a committed standard is put back with its other notes"
    else
      bad "create-only: a timed-out pass's edit to a committed standard was left or recorded -- log: $(tr '\n' '|' < "$PROMO_LOG" | cut -c1-2000)"
    fi
  else
    bad "create-only timeout control: $CO_NOTE is not a clean note committed without a Vault-Pass trailer, so the control did not run"
  fi
  co_restore "$CO_NOTE"
  rm -f "$RV/31-standards/beside-hang.md"

  # A 1.2.0 pass could leave an uncommitted edit of a committed note for the next
  # run to adopt. The first run under create-only puts it back before its agent
  # starts, keeping a copy, so it cannot sink that run's own notes.
  co_case promotion-create-only-leftover
  rm -f "$TMP/co-seen"
  if co_owner_note "$CO_NOTE" && co_owner_note "$CO_WIKI"; then
    printf 'an earlier pass edit\n' >> "$RV/$CO_NOTE"
    printf 'an earlier pass edit\n' >> "$RV/$CO_WIKI"
    mkdir -p "$CASE_STATE"
    for p in "$CO_NOTE" "$CO_WIKI"; do
      printf '%s\t%s\n' "$(git -C "$RV" hash-object --no-filters -- "$p")" "$p"
    done > "$CASE_STATE/promotion-pass.uncommitted"
  fi
  co_records="$(awk -F '\t' '$1 ~ /^[0-9a-f]+$/ && length($1) >= 40 { n++ } END { print n + 0 }' "$CASE_STATE/promotion-pass.uncommitted" 2>/dev/null)"
  if [ "${co_records:-0}" -eq 2 ] \
     && ! git -C "$RV" diff --quiet HEAD -- "$CO_NOTE" && ! git -C "$RV" diff --quiet HEAD -- "$CO_WIKI"; then
    ran promo-create-only-leftover
    expect_rc "promotion-pass: the first run after a pass left edits of committed notes uncommitted -> OK" 0 \
      "$(RUNNER_NO_SETTLE=1 runner promotion-pass.sh summary FAKE_SEE="$CO_NOTE" FAKE_SEEN="$TMP/co-seen")"
    co_lcopy="$(find "$CASE_STATE/quarantine" -path "*-leftover/$CO_NOTE" -type f 2>/dev/null | head -n 1)"
    co_wcopy="$(find "$CASE_STATE/quarantine" -path "*-leftover/$CO_WIKI" -type f 2>/dev/null | head -n 1)"
    if grep -q 'LEFTOVER-REJECTED' "$PROMO_LOG" \
       && grep -qxF "    create-only: $CO_NOTE is a long-tier note the last commit already holds, and a pass may only add notes there" "$PROMO_LOG" \
       && grep -qxF "    create-only: $CO_WIKI is a long-tier note the last commit already holds, and a pass may only add notes there" "$PROMO_LOG" \
       && [ -f "$TMP/co-seen" ] && grep -qx 'existing' "$TMP/co-seen" && ! grep -q 'an earlier pass edit' "$TMP/co-seen" \
       && [ "$(git -C "$RV" rev-parse HEAD)" = "$co_head" ] && tree_matches_head "$RV" "$CO_NOTE" && tree_matches_head "$RV" "$CO_WIKI" \
       && [ -n "$co_lcopy" ] && grep -qx 'an earlier pass edit' "$co_lcopy" \
  ...
  co_restore "$CO_NOTE"
  co_restore "$CO_WIKI"
  # The wiki folder goes back to empty, which the tier folder control below needs.
  git -C "$RV" rm -q -f --ignore-unmatch -- "$CO_WIKI" >/dev/null 2>&1
  git -C "$RV" commit -q --no-verify -m "remove the create-only wiki fixture" -- "$CO_WIKI" >/dev/null 2>&1
  rm -f "$RV/$CO_WIKI"
  mkdir -p "$RV/40-llm-wiki/wiki"
  if [ -n "$(ls -A "$RV/40-llm-wiki/wiki")" ] || git -C "$RV" rev-parse -q --verify "HEAD:$CO_WIKI" >/dev/null 2>&1; then
    bad "create-only teardown: the wiki fixture $CO_WIKI is still on disk or in HEAD, so the tier folder control below cannot run"
  fi

  # A note the pass wrote and something else committed during the pass is new:
  # HEAD after the pass holds it, the commit before the pass does not. The check
  # reads the commit before the pass, or it would refuse a sync plugin's work.
  co_case promotion-create-only-syncnew
  if [ ! -e "$RV/31-standards/synced-new.md" ] \
     && ! git -C "$RV" rev-parse -q --verify "HEAD:31-standards/synced-new.md" >/dev/null 2>&1; then
    ran promo-create-only-syncnew
    expect_rc "promotion-pass: the pass writes a new standard and something commits it during the pass -> OK" 0 \
      "$(runner promotion-pass.sh promote-syncnew)"
    if [ "$(git -C "$RV" log -1 --format=%s 2>/dev/null)" = "sync plugin" ] \
       && [ "$(git -C "$RV" rev-parse -q --verify 'HEAD^' 2>/dev/null)" = "$co_head" ] \
       && git -C "$RV" rev-parse -q --verify "HEAD:31-standards/synced-new.md" >/dev/null 2>&1 \
       && grep -qF 'NOTE: HEAD already holds 31-standards/synced-new.md' "$PROMO_LOG" \
       && ! grep -q 'VIOLATION' "$PROMO_LOG"; then
      ok "create-only: a new note something committed during the pass is not taken for one that was there before"
    else
      bad "create-only: a new note committed during the pass was refused, or never committed -- log: $(tr '\n' '|' < "$PROMO_LOG" | cut -c1-2000)"
    fi
  else
    bad "create-only syncnew control: 31-standards/synced-new.md already exists, so the control did not run"
  fi

  # Either test of "there before" must hold on its own, which no pass run can
  # show: every note a pass reaches is on disk, so the snapshot lists it and the
  # snapshot test alone refuses it. check_owned is called directly here, on a
  # note HEAD holds, with a snapshot that does not list it.
  # co_unit <HEAD_BEFORE> <snapshot: another|note|no> <adopted: yes|no> [<path>]
  # The snapshot lists another note, lists the fixture note, or is missing.
  co_unit() {
    local d="$TMP/co-unit" p="${4:-$CO_NOTE}"
    rm -rf "$d"
    mkdir -p "$d"
    printf '%s\n' "$p" > "$d/owned"
    : > "$d/predirty"
    : > "$d/log"
    : > "$d/predirty.adopted"
    [ "$3" = yes ] && printf '%s\n' "$p" > "$d/predirty.adopted"
    [ "$2" = another ] && printf '1 1 ./31-standards/another.md\n' > "$d/before"
    [ "$2" = note ] && printf '1 1 ./%s\n' "$CO_NOTE" > "$d/before"
    ( . "$ROOT/.claude/scripts/lib/runner-common.sh" && HEAD_BEFORE="$1" \
        && check_owned "$RV" "$d/owned" "$d/predirty" "$d" "$d/log" ) >/dev/null 2>&1
    co_unit_rc=$?
    # The refusal line for the path says which test refused it.
    co_line="$(H="$CO_TEXT" P="    $p" awk 'index($0, ENVIRON["H"]) { f = 1; next } f && /^    / { if ($0 == ENVIRON["P"] || index($0, ENVIRON["P"] " (") == 1) { print; exit } next } { f = 0 }' "$d/log" 2>/dev/null)"
    if [ "$co_unit_rc" -eq 2 ] && [ -n "$co_line" ]; then
      case "$co_line" in
        "    $p") co_unit_said=refused ;;
        *"(its name differs only in case"*) co_unit_said=refused-case ;;
        *"(there is no snapshot"*) co_unit_said=refused-nosnap ;;
      ...
      co_unit_said="rc $co_unit_rc: $(tr '\n' '|' < "$d/log" | cut -c1-200)"
    fi
  }
  co_sha="$(git -C "$RV" rev-parse -q --verify HEAD 2>/dev/null)"
  if co_owner_note "$CO_NOTE" && [ -n "$co_sha" ]; then
    ran promo-create-only-arms
    co_unit "refs/heads/unit $co_sha" another no
    co_a="$co_unit_said"
    co_unit "refs/heads/unit $co_sha" another yes
    co_b="$co_unit_said"
    co_unit "refs/heads/unit NONE" no no
    co_c="$co_unit_said"
    co_unit "refs/heads/unit NONE" another no
    co_d="$co_unit_said"
    # A name that differs from the note only in case. On a filesystem that folds
    # case it already names the note; elsewhere a file is made for it, and only
    # that file is removed afterwards.
    co_twin=31-standards/EXISTING.md
    co_twin_made=0
    if [ ! -e "$RV/$co_twin" ]; then
      printf 'twin\n' > "$RV/$co_twin"
      co_twin_made=1
    fi
    co_unit "refs/heads/unit NONE" note no "$co_twin"
    co_e="$co_unit_said"
    co_unit "refs/heads/unit $co_sha" another no "$co_twin"
    co_f="$co_unit_said"
    # An earlier pass's leftover is exempt, but not when it is a case twin.
    co_unit "refs/heads/unit $co_sha" another yes "$co_twin"
    co_g="$co_unit_said"
    [ "$co_twin_made" -eq 1 ] && ! [ "$RV/$co_twin" -ef "$RV/$CO_NOTE" ] && rm -f "$RV/$co_twin"
  ...
  # "Whoever wrote it" includes a promotion pass: a note a pass committed, with
  # the trailers the runner writes, is as closed to the next pass as one the
  # owner wrote.
  pn_note=31-standards/pass-written.md
  if [ ! -e "$RV/$pn_note" ]; then
    printf -- '---\ntier: long\ntype: standard\n---\n\nwritten by a pass\n' > "$RV/$pn_note"
    printf 'promotion pass: %s\n\nVault-Pass: promotion\nVault-Pass-Blob: %s %s\n' "$pn_note" \
      "$(git -C "$RV" hash-object -- "$pn_note" 2>/dev/null)" "$pn_note" > "$TMP/pn-msg"
    git -C "$RV" add -- "$pn_note" >/dev/null 2>&1
    git -C "$RV" commit -q --no-verify -F "$TMP/pn-msg" -- "$pn_note" >/dev/null 2>&1
  fi
  co_case promotion-create-only-passnote
  pn_msg="$(git -C "$RV" log -1 --format=%B -- "$pn_note" 2>/dev/null)"
  pn_blob="$(git -C "$RV" rev-parse -q --verify "HEAD:$pn_note" 2>/dev/null)"
  if [ -n "$pn_blob" ] && tree_matches_head "$RV" "$pn_note" \
     && printf '%s\n' "$pn_msg" | grep -qx 'Vault-Pass: promotion' \
     && printf '%s\n' "$pn_msg" | grep -qxF "Vault-Pass-Blob: $pn_blob $pn_note"; then
    ran promo-create-only-passnote
    expect_rc "promotion-pass: the pass appends to a note an earlier promotion pass committed -> VIOLATION" 2 \
      "$(runner promotion-pass.sh promote-append FAKE_APPEND="$pn_note")"
    co_copy="$(co_rejected "$pn_note")"
    if co_named "$pn_note" && [ "$(git -C "$RV" rev-parse HEAD)" = "$co_head" ] && tree_matches_head "$RV" "$pn_note" \
       && [ -n "$co_copy" ] && grep -qx 'promoted again' "$co_copy"; then
      ok "create-only: a note an earlier promotion pass committed is refused like one the owner wrote"
    else
      bad "create-only: an edit to a pass-committed note was not refused and put back -- log: $(tr '\n' '|' < "$PROMO_LOG" | cut -c1-2000)"
    fi
  else
    bad "create-only passnote control: $pn_note is not a clean note whose last commit carries the runner's trailers, so the control did not run"
  fi
  co_restore "$pn_note"

  # A new note whose name is a glob matching a committed sibling is still a new
  # note, and each check takes its name literally: it fails vault-check on its
  # own bytes, so the pass exits 5 with no create-only refusal and the sibling
  # is left as it was.
  co_case promotion-create-only-globnew
  gn_note='31-standards/[p]ass-written.md'
  if [ -n "$pn_blob" ] && tree_matches_head "$RV" "$pn_note" && [ ! -e "$RV/$gn_note" ]; then
    ran promo-create-only-globnew
    expect_rc "promotion-pass: the pass writes a malformed new note named as a pattern matching a committed sibling -> CHECK-FAILED" 5 \
      "$(runner promotion-pass.sh promote-glob-new)"
    gn_copy=""
    for q in "$CASE_STATE"/quarantine/*-rejected; do
      [ -f "$q/$gn_note" ] && gn_copy="$q/$gn_note"
    done
    if grep -q 'CHECK-FAILED' "$PROMO_LOG" && ! grep -qF "$CO_TEXT" "$PROMO_LOG" \
       && [ ! -e "$RV/$gn_note" ] && [ -n "$gn_copy" ] && grep -qx 'no frontmatter' "$gn_copy" \
       && [ "$(git -C "$RV" rev-parse HEAD)" = "$co_head" ] && tree_matches_head "$RV" "$pn_note"; then
      ok "create-only: a new note named as a pattern is checked by its own name, and its committed sibling is untouched"
    else
      bad "create-only: a new note named as a pattern was refused as create-only or checked as its sibling -- log: $(tr '\n' '|' < "$PROMO_LOG" | cut -c1-2000)"
    fi
  else
    bad "create-only globnew control: the committed sibling $pn_note is missing or dirty, or $gn_note exists, so the control did not run"
  fi
  rm -f "$RV/$gn_note"
  ...
  for ct_pair in "$CO_NOTE 31-standards/EXISTING.md" "31-standards/Upper-Note.md 31-standards/upper-note.md"; do
    ct_note="${ct_pair%% *}"
    ct_twin="${ct_pair#* }"
    co_case "promotion-create-only-casetwin-$(basename "$ct_twin" .md)"
    if co_owner_note "$ct_note" && ls "$RV/31-standards" | grep -qxF "$(basename "$ct_note")" \
       && ! ls "$RV/31-standards" | grep -qxF "$(basename "$ct_twin")"; then
      ct_ok=$((ct_ok + 1))
      expect_rc "promotion-pass: the pass replaces $ct_note under the name $ct_twin -> VIOLATION" 2 \
        "$(runner promotion-pass.sh promote-case-twin FAKE_TWIN="$ct_twin")"
      # By exact name, which [ -f ] and git cannot tell on a filesystem that
      # folds case: the note is listed under its own name, and the twin is gone.
      # The pass's bytes are in the quarantine under the name they ended up
      # with: the twin's on NTFS and on Linux, the note's on APFS, where a
      # rename over a name that differs only in case keeps the old name.
      ct_copy="$(co_rejected "$ct_twin")"
      [ -n "$ct_copy" ] || ct_copy="$(co_rejected "$ct_note")"
      if { co_named "$ct_twin" || co_named "$ct_note"; } && [ -f "$RV/$ct_note" ] && tree_matches_head "$RV" "$ct_note" \
         && ls "$RV/31-standards" | grep -qxF "$(basename "$ct_note")" \
         && ! ls "$RV/31-standards" | grep -qxF "$(basename "$ct_twin")" \
         && [ -n "$ct_copy" ] && grep -qx 'case twin' "$ct_copy" \
  ...
  if [ "$ct_ok" -eq 2 ]; then
    ran promo-create-only-casetwin
    if [ "$ct_said" = " ok ok" ]; then
      ok "create-only: a note replaced under a name that differs only in case is refused and put back under its own name, in either order"
    else
      bad "create-only: a case twin was committed, or its note was not put back --$ct_said"
    fi
  else
  ...
  # A pass that overwrites a committed standard is refused as create-only before
  # vault-check reads anything, and has every note it changed put back: a
  ...
  : > "$PROMO_LOG"
  expect_rc "promotion-pass: the pass overwrites a committed standard and writes a note that fails vault-check -> VIOLATION" 2 "$(runner promotion-pass.sh promote-bad)"
  if [ "$(git -C "$RV" rev-parse HEAD)" = "$pb_head" ] && tree_matches_head "$RV" 31-standards \
     ...
     && grep -qF "$CO_TEXT" "$PROMO_LOG" && ! grep -q 'CHECK-FAILED' "$PROMO_LOG" && grep -q 'REVERTED' "$PROMO_LOG" \
  ...
  # CHECK-FAILED puts back a committed note the pass may change, which a
  # promotion report still is: restored from the commit before the pass, with
  # the rejected bytes kept, and the pass's new note moved out.
  pr_note=20-projects/_logs/promotion-committed-report.md
  printf -- '---\ntier: medium\ntype: project-log\n---\n\nreport\n' > "$RV/$pr_note"
  settle_owned "$RV"
  pr_head="$(git -C "$RV" rev-parse HEAD)"
  new_case_state promotion-check-fails-report
  : > "$PROMO_LOG"
  if git -C "$RV" rev-parse -q --verify "HEAD:$pr_note" >/dev/null 2>&1 && tree_matches_head "$RV" "$pr_note"; then
    expect_rc "promotion-pass: the pass overwrites a committed promotion report and writes a note, both failing vault-check -> CHECK-FAILED" 5 \
      "$(runner promotion-pass.sh promote-bad FAKE_OVERWRITE="$pr_note")"
    pr_copy="$(find "$CASE_STATE/quarantine" -path "*-rejected/$pr_note" -type f 2>/dev/null | head -n 1)"
    if [ "$(git -C "$RV" rev-parse HEAD)" = "$pr_head" ] && tree_matches_head "$RV" "$pr_note" \
       && tree_matches_head "$RV" 31-standards && [ ! -e "$RV/31-standards/rejected-new.md" ] \
       && grep -q 'CHECK-FAILED' "$PROMO_LOG" && ! grep -qF "$CO_TEXT" "$PROMO_LOG" \
       && grep -qF "$pr_note (copied to " "$PROMO_LOG" \
       && [ -n "$pr_copy" ] && grep -q 'overwritten without frontmatter' "$pr_copy"; then
      ok "a committed promotion report that fails vault-check is restored from the commit before the pass, its rejected bytes kept"
    else
      bad "a committed promotion report that fails vault-check was not put back -- log: $(tr '\n' '|' < "$PROMO_LOG" | cut -c1-2000)"
    fi
  else
    bad "the CHECK-FAILED report case: $pr_note is not committed and clean, so the case did not run"
  fi
  git -C "$RV" checkout -q -- "$pr_note"
  rm -f "$RV/31-standards/rejected-new.md"
  ...
  : > "$PROMO_LOG"
  expect_rc "promotion-pass: rejected notes and an edited note git ignores -> VIOLATION" 2 "$(runner promotion-pass.sh promote-bad)"
  if grep -q '^keep$' "$RV/31-standards/ignored-keep.md" 2>/dev/null && grep -q '^pass edit$' "$RV/31-standards/ignored-keep.md" \
     ...
     && co_named 31-standards/existing.md && co_named 31-standards/ignored-keep.md \
  ...
  # restore over someone's uncommitted edit to existing.md. The bracketed note
  # is committed before the pass, so the pass is refused as create-only.
  printf -- '---\ntier: long\ntype: standard\n---\n\nbracketed\n' > "$RV/31-standards/[e]xisting.md"
  settle_owned "$RV"
  printf 'a human edit\n' >> "$RV/31-standards/existing.md"
  new_case_state promotion-revert-literal
  : > "$PROMO_LOG"
  expect_rc "promotion-pass: a rejected bracketed note next to a dirty sibling -> VIOLATION" 2 \
    "$(RUNNER_NO_SETTLE=1 runner promotion-pass.sh promote-glob-bad)"
  if grep -q '^a human edit' "$RV/31-standards/existing.md" && grep -q '^bracketed' "$RV/31-standards/[e]xisting.md" \
     && co_named '31-standards/[e]xisting.md' && ! co_named 31-standards/existing.md; then
  ...
  : > "$PROMO_LOG"
  expect_rc "promotion-pass: a rejected pass whose note was committed during it -> VIOLATION" 2 \
...
# "A COPY PLAN WAS PRINTED", which is one sentence and not two. The two
# spellings are one letter apart and do not mean the same thing. The copy
# plan's heading says "Safe to take (N)." with a capital and appears only when
# there is something to copy. The counts line says "N safe to take," in lower
# case and is printed by every run that reports at all, INCLUDING ONE WHOSE
# NUMBER IS ZERO.
#
# Matching both was wrong in a way that hid itself. "0 safe to take," contains
# " safe to take,", so a run that reported honestly and offered nothing read as
# a run that had offered something, and this helper answered the opposite of
# its own name. That never showed, because every one of its nine uses is a
# NEGATIVE - `vu_offered && vu_bad=...` on a refusal - and a refusal prints
# neither sentence. A negative use can only ever be weakened by a helper that
# matches less, so those nine could not hold this and were never going to.
#
# A mutation dropping the capitalised arm survived the whole suite for that
# reason, which is how the defect was found. `tmpl-offered-is-a-plan` is the
# positive use that holds it now, and the arm that does the holding is the one
# where a report runs and offers nothing.
vu_offered() {
  grep -qF -- 'Safe to take (' "$VU_OUT"
}
...
if ! command -v git >/dev/null 2>&1; then
  ...
  skip tmpl-offered-is-a-plan "the template update controls build git fixtures, and git is not installed"
...
if [ "${vu_base_entries:-0}" != 6 ]; then
  ...
  # -- what "it offered something" is allowed to mean ----------------------

  # THE SUITE'S OWN HELPER, held for the first time. `vu_offered` is the
  # predicate nine refusal controls use to say no copy plan was printed, and it
  # used to match the counts line as well as the plan's heading. Those two are
  # one letter apart and do not mean the same thing, and "0 safe to take,"
  # contains the lower-case one, so a report that ran honestly and offered
  # nothing read as a report that had offered something.
  #
  # Nothing could catch that, and the reason is structural rather than an
  # oversight. Every use of the helper is a NEGATIVE, and a negative assertion
  # only gets weaker when its predicate matches less, so all nine stayed green
  # against a helper with an arm removed. A mutation that dropped one arm
  # survived the whole suite, which is how this was found. Holding a predicate
  # takes a POSITIVE use, and the arm that does the work is this first one,
  # where the report runs and there is nothing to copy.
  vu_op="$VU/offered-nothing"
  vu_make "$vu_op" 1.0.0
  vu_op_src="$VU/offered-nothing-src"
  vu_make "$vu_op_src" 1.1.0
  # The only file two prototype versions differ in is VERSION, so changing it
  # here as well turns the one thing that moved upstream into a merge and
  # leaves nothing at all safe to take. Not regenerated afterwards, because
  # "changed here" is measured against the manifest and regenerating would
  # restamp it.
  printf '1.0.1\n' > "$vu_op/VERSION"
  vu_bad=''
  vu_rc_op="$(vu_rc "$vu_op" --check --from "$vu_op_src")"
  # Measured out of the output rather than assumed, because all three of these
  # have to hold for the assertion below to be asking the question it names.
  # The report has to have run, its number has to be zero, and no plan may have
  # been printed.
  vu_op_counts="$(LC_ALL=C awk '/ safe to take,/ { n++ } END { print n + 0 }' "$VU_OUT" 2>/dev/null)"
  vu_op_zero="$(LC_ALL=C awk '/ 0 safe to take,/ { n++ } END { print n + 0 }' "$VU_OUT" 2>/dev/null)"
  vu_op_plan="$(LC_ALL=C awk '/Safe to take \(/ { n++ } END { print n + 0 }' "$VU_OUT" 2>/dev/null)"
  [ "$vu_rc_op" = 10 ] || vu_bad="$vu_bad offered-nothing-rc:$vu_rc_op"
  [ "${vu_op_counts:-0}" -ge 1 ] 2>/dev/null \
    || vu_bad="$vu_bad no-counts-line-so-the-report-never-ran-[${vu_op_counts:-unmeasurable}]"
  [ "${vu_op_zero:-0}" -ge 1 ] 2>/dev/null \
    || vu_bad="$vu_bad the-counts-line-does-not-say-zero-safe-to-take-so-something-was-offered"
  [ "${vu_op_plan:-1}" = 0 ] || vu_bad="$vu_bad a-copy-plan-was-printed-so-there-was-something-to-offer"
  # THE ASSERTION. A run that printed no plan must not read as having offered
  # one, whatever else it printed.
  vu_offered && vu_bad="$vu_bad a-report-that-offered-nothing-read-as-having-offered-something"

  # And the other direction, or the helper could be made to answer no to
  # everything and the arm above would still pass. Every wording about the run
  # above is asserted before this one starts, because the helper and vu_says
  # both read one shared output file.
  vu_op2="$VU/offered-something"
  vu_make "$vu_op2" 1.0.0
  vu_op2_src="$VU/offered-something-src"
  vu_make "$vu_op2_src" 1.1.0
  vu_rc_op2="$(vu_rc "$vu_op2" --check --from "$vu_op2_src")"
  vu_op2_plan="$(LC_ALL=C awk '/Safe to take \(/ { n++ } END { print n + 0 }' "$VU_OUT" 2>/dev/null)"
  [ "$vu_rc_op2" = 10 ] || vu_bad="$vu_bad offered-something-rc:$vu_rc_op2"
  [ "${vu_op2_plan:-0}" -ge 1 ] 2>/dev/null \
    || vu_bad="$vu_bad the-second-fixture-printed-no-copy-plan-[${vu_op2_plan:-unmeasurable}]-so-the-positive-arm-proves-nothing"
  vu_offered || vu_bad="$vu_bad a-report-that-printed-a-copy-plan-did-not-read-as-having-offered-one"

  ran tmpl-offered-is-a-plan
  if [ -z "$vu_bad" ]; then
    ok "the suite's own test for whether an update was offered answers yes to a printed copy plan and no to a report that ran and had nothing to copy, rather than yes to both"
  else
    bad "the suite's test for whether an update was offered does not mean what its name says --$vu_bad [$(vu_excerpt)]"
  fi
  ...
  if [ ! -f "$VU_REL" ]; then
    ...
    # -- a tag git would not write -----------------------------------------

    # THE ONE DOOR IN THIS FILE NOTHING HAS EVER OPENED. TAG-FAILED had no
    # control at all, which is recorded rather than assumed, and the arm above
    # hands in an identity precisely so that it never arrives here. That left
    # its exit code free to be anything, and it was a 1 - a release is owed -
    # for what is the tool failing rather than a finding about the release.
    #
    # It matters more than an unexercised message usually would. An arm that
    # expects a refusal BEFORE any tag is written sees a run with no identity
    # reach git, take this door, write no tag and leave on 1, which is exactly
    # the shape such an arm looks for. On a runner, which has no identity, that
    # arm would have been green while the defect it exists to catch stood. The
    # code is a 2 now and this is what holds it there.
    #
    # The state is a runner's, reproduced by pointing git at an empty config
    # for both the global and the system file and clearing the four identity
    # variables. A subshell, so the clearing cannot reach anything after it.
    vu_tf="$VU/release-tagfailed"
    vu_make "$vu_tf" 1.0.0
    printf '# Changelog\n\n## 1.0.0 - 2026-01-01\n\nThe first one.\n\n### Adopting this\n\nNothing to do.\n' > "$vu_tf/CHANGELOG.md"
    vu_git "$vu_tf"
    vu_tag "$vu_tf" 1.0.0
    ...
    if [ "$vu_tf_ident" = 0 ] || [ "${vu_tf_heads:-0}" != 1 ] || [ "${vu_tf_tagged:-1}" != 0 ] \
      ...
      vu_tf_rc="$( unset GIT_AUTHOR_NAME GIT_AUTHOR_EMAIL GIT_COMMITTER_NAME GIT_COMMITTER_EMAIL EMAIL
        cd "$vu_tf" && CLAUDE_PROJECT_DIR="$vu_tf" LC_ALL=C \
        GIT_CONFIG_GLOBAL="$vu_tf_empty" GIT_CONFIG_SYSTEM="$vu_tf_empty" \
        "$VU_BASH" "$VU_REL" --tag > "$VU_OUT" 2>&1; printf '%s' "$?" )"
      vu_tf_after="$(git -C "$vu_tf" tag -l 2>/dev/null | LC_ALL=C awk '$0 == "2.0.0" { n++ } END { print n + 0 }')"
      # git's own complaint reaches the reader rather than being swallowed,
      # which is the only thing that tells them which tool failed and why. The
      # relayed text is counted by its SHAPE, as an indented line that is not
      # one of this script's own, rather than by quoting a sentence of git's.
      # git translates these and its wording moves between versions, so an
      # assertion naming the sentence would fail on a runner for a reason that
      # has nothing to do with the release check.
      vu_tf_relayed="$(LC_ALL=C awk '/^  / && $0 !~ /^release-check:/ { n++ } END { print n + 0 }' "$VU_OUT" 2>/dev/null)"
      # TWO, because git would not write the tag is the tool failing and this
      ...
      # would mean this never reached the door it names.
      vu_says 'NO-NOTES' && vu_bad="$vu_bad stopped-at-the-notes-instead"
      vu_says 'ALREADY-TAGGED' && vu_bad="$vu_bad stopped-at-the-existing-tag-instead"
      vu_says 'VERSION-GOES-BACKWARD' && vu_bad="$vu_bad stopped-at-the-ordering-instead"
      ran tmpl-release-tag-failed
      if [ -z "$vu_bad" ]; then
        ok "a checkout that cannot say who the tagger is leaves by the door that says the tool failed rather than the one that says a release is owed, prints what git said about it, and writes no tag"
      else
        bad "a tag git would not write was not reported as a tool failure --$vu_bad [$(vu_excerpt)]"
      fi
    fi
    ...
    for vu_vs_case in '1.2.0-rc1' '1.2.0 ' '1.1.0^' '1..2' '.1.2'; do
      ...
      # TWO, NOT ONE, and the difference is the point of having two codes. A
      # version nothing here can read means nothing could be compared, which is
      # what a 2 says, and the absent-VERSION refusal it sits beside has always
      # been a 2 for the same defect with the value missing rather than
      # misspelled.
      [ "$vu_rc_vs" = 2 ] || vu_bad="$vu_bad [$vu_vs_case]rc:$vu_rc_vs"
      vu_says 'VERSION-SPELLING' || vu_bad="$vu_bad [$vu_vs_case]no-reason"
      vu_says 'NOT saying the release is up to date' || vu_bad="$vu_bad [$vu_vs_case]did-not-say-what-it-was-not-saying"
      ...
      vu_says 'UNRELEASED-CHANGES' && vu_bad="$vu_bad [$vu_vs_case]called-it-an-owed-release"
    done
    ...
    # -- a scratch directory that was not emptied --------------------------

    # THE EMPTINESS ASSERTION, which had no control and was recorded as having
    # none rather than assumed to be covered. `mkdir -p` returns 0 for a
    # directory that is already there, so it cannot tell a failed clear from a
    # clean one, and the `rm -rf` above it has its status read nowhere. Four of
    # the files the run then writes have their redirection status read nowhere
    # either, so a survivor would be read as this run's data and the answer
    # would come out of a previous run's files.
    #
    # A STAND-IN rm THAT REMOVES NOTHING AND REPORTS SUCCESS, rather than a
    # permission trick. Which trick makes a real rm fail is not the same on the
    # three platforms here, and it was measured rather than guessed - on
    # Windows an unwritable scratch directory, an unenterable subdirectory, the
    # read-only attribute and an unwritable parent all still let rm -rf
    # succeed - so any of them would have given a control that runs on some
    # jobs and skips on the rest. The stand-in also models the case that
    # actually matters, which is not a clear that fails loudly but one that
    # says it worked while the state did not change, because that status is
    # precisely the one nothing reads.
    vu_sc="$VU/release-scratch"
    vu_make "$vu_sc" 1.0.0
    printf '# Changelog\n\n## 1.0.0 - 2026-01-01\n\nThe first one.\n\n### Adopting this\n\nNothing to do.\n' > "$vu_sc/CHANGELOG.md"
    vu_git "$vu_sc"
    vu_tag "$vu_sc" 1.0.0
    vu_sc_shim="$VU/release-scratch-shim"
    rm -rf "$vu_sc_shim"
    mkdir -p "$vu_sc_shim"
    {
      printf '#!/usr/bin/env bash\n'
      printf '# A stand-in rm that removes nothing and says it succeeded, so that the\n'
      printf '# release check meets a scratch directory its clear did not clear.\n'
      printf '# Written by the control suite into a temporary folder, never shipped.\n'
      printf 'printf "stood in for rm\\n" >> "$VAULT_RM_MARKER"\n'
      printf 'exit 0\n'
    } > "$vu_sc_shim/rm"
    chmod +x "$vu_sc_shim/rm" 2>/dev/null
    vu_sc_scratch="$vu_sc/.git/release-check"
    mkdir -p "$vu_sc_scratch"
    printf 'a previous run left this behind\n' > "$vu_sc_scratch/shipped"
    # Whether the stand-in is reached is established by RUNNING it and reading
    # what it left, not by asking command -v. bash keeps a hash table of
    # command locations and consults it before PATH, and this suite has run rm
    # many times by now. The hash is dropped as well, but the evidence is the
    # probe file surviving a clear and the marker being written.
    hash -r 2>/dev/null || true
    vu_sc_marker="$VU/release-scratch.marker"
    : > "$vu_sc_marker"
    vu_sc_probe="$VU/release-scratch.probe"
    printf 'probe\n' > "$vu_sc_probe"
    ( PATH="$vu_sc_shim:$PATH" VAULT_RM_MARKER="$vu_sc_marker" rm -rf "$vu_sc_probe" ) >/dev/null 2>&1
    vu_sc_left=0
    [ -e "$vu_sc_probe" ] && vu_sc_left=1
    vu_sc_marked=0
    [ -s "$vu_sc_marker" ] && vu_sc_marked=1
    rm -f "$vu_sc_probe" 2>/dev/null
    vu_sc_planted="$(LC_ALL=C awk '/a previous run left this behind/ { n++ } END { print n + 0 }' "$vu_sc_scratch/shipped" 2>/dev/null)"
    if [ "$vu_sc_left" != 1 ] || [ "$vu_sc_marked" != 1 ] || [ "${vu_sc_planted:-0}" != 1 ]; then
      skip tmpl-release-scratch-survivor "a clear that removes nothing could not be arranged here -- the stand-in left the probe:$vu_sc_left wrote its marker:$vu_sc_marked and the survivor was planted:${vu_sc_planted:-unmeasurable}, so the refusal's silence would prove nothing"
    else
      vu_bad=''
      vu_sc_rc="$( cd "$vu_sc" && PATH="$vu_sc_shim:$PATH" VAULT_RM_MARKER="$vu_sc_marker" \
        CLAUDE_PROJECT_DIR="$vu_sc" "$VU_BASH" "$VU_REL" > "$VU_OUT" 2>&1; printf '%s' "$?" )"
      # The survivor is still the survivor. This is the assertion that fails
      # when the guard is deleted, because a run that goes past it writes its
      # own shipped list over exactly this file.
      vu_sc_after="$(LC_ALL=C awk '/a previous run left this behind/ { n++ } END { print n + 0 }' "$vu_sc_scratch/shipped" 2>/dev/null)"
      [ "$vu_sc_rc" = 2 ] || vu_bad="$vu_bad scratch-rc:$vu_sc_rc"
      vu_says 'NO-SCRATCH' || vu_bad="$vu_bad scratch-gave-no-reason"
      # WHICH of the NO-SCRATCH cases, because that name covers a directory
      # that could not be made and three counts that could not be taken, and
      # collapsing any of them into another would leave this green.
      vu_says 'could not be emptied' || vu_bad="$vu_bad scratch-did-not-say-which-NO-SCRATCH-case-it-was"
      vu_says 'nothing is owed' && vu_bad="$vu_bad scratch-reported-the-release-as-up-to-date"
      vu_says 'UNRELEASED-CHANGES' && vu_bad="$vu_bad scratch-answered-about-the-release-at-all"
      [ "${vu_sc_after:-0}" = 1 ] || vu_bad="$vu_bad the-survivor-was-overwritten-so-the-run-went-past-the-guard"
      ran tmpl-release-scratch-survivor
      if [ -z "$vu_bad" ]; then
        ok "a scratch directory whose clear removed nothing is refused as a previous run's files rather than read as this run's, and the run answers nothing at all about the release"
      else
        bad "a scratch the clear did not clear was answered from --$vu_bad [$(vu_excerpt)]"
      fi
    fi

    # -- the two sides of SHIPPED-UNKNOWN, and a mismatch that is not total --

    # TWO GAPS THAT MEET. The guard used to fire only when a whole side matched
    # NOTHING, and the arm that exercised it gave a bad spelling only to THIS
    # TREE'S manifest. So the tag half could never fire for want of a fixture,
    # and neither half could fire for one mangled path among many - which is
    # the shape that actually happens, because a leading dot-slash is somebody
    # rewriting a manifest by hand and a case-folded path is a manifest
    # regenerated on an ordinary macOS or Windows machine.
    vu_bad=''
    vu_su="$VU/release-unknown-tag"
    vu_make "$vu_su" 1.0.0
    printf '# Changelog\n\n## 1.0.0 - 2026-01-01\n\nThe first one.\n\n### Adopting this\n\nNothing to do.\n' > "$vu_su/CHANGELOG.md"
    vu_git "$vu_su"
    ...
    vu_tag "$vu_su" 1.0.0
    LC_ALL=C awk '
      ($1 == "owned" || $1 == "seed") { p = $3; sub(/^\.\//, "", p); printf "%s %s %s\n", $1, $2, p; next }
      { print }
    ' "$vu_su/.claude/template-manifest" > "$vu_su/.claude/template-manifest.new"
    mv "$vu_su/.claude/template-manifest.new" "$vu_su/.claude/template-manifest"
    vu_git "$vu_su"
    ...
    # And a mismatch that is not total, on both sides at once. One path spelled
    # with a different case from git's index, folded BEFORE the tag so both
    # manifests carry it - which is the state a repository whose manifests were
    # always generated on a case-insensitive filesystem is in. Folding one side
    # only would prove nothing, because the other side still names the path,
    # the union still holds it and the ordinary comparison finds the change.
    vu_pu="$VU/release-unknown-partial"
    vu_make "$vu_pu" 1.0.0
    printf '# Changelog\n\n## 1.0.0 - 2026-01-01\n\nThe first one.\n\n### Adopting this\n\nNothing to do.\n' > "$vu_pu/CHANGELOG.md"
    LC_ALL=C awk '{ sub(/ docs\/a\.md$/, " Docs/a.md"); print }' \
      "$vu_pu/.claude/template-manifest" > "$vu_pu/.claude/template-manifest.new"
    mv "$vu_pu/.claude/template-manifest.new" "$vu_pu/.claude/template-manifest"
    vu_git "$vu_pu"
    vu_tag "$vu_pu" 1.0.0
    # And the file both manifests misspell really does change, so a release is
    # genuinely owed and a clean answer here is the defect rather than a
    # difference of opinion about how strict to be.
    printf 'doc a, edited after the release went out\n' > "$vu_pu/docs/a.md"
    vu_git "$vu_pu"
    vu_pu_folded="$(LC_ALL=C awk '$3 == "Docs/a.md" { n++ } END { print n + 0 }' "$vu_pu/.claude/template-manifest" 2>/dev/null)"
    vu_pu_tracked="$(git -C "$vu_pu" ls-files 2>/dev/null | LC_ALL=C awk '$0 == "docs/a.md" { n++ } END { print n + 0 }')"
    vu_pu_ships="$(LC_ALL=C awk '$1 == "owned" || $1 == "seed" { n++ } END { print n + 0 }' "$vu_pu/.claude/template-manifest" 2>/dev/null)"
    vu_pu_rc="$(vu_rel "$vu_pu")"
    [ "${vu_pu_folded:-0}" = 1 ] || vu_bad="$vu_bad the-fold-did-not-land-[${vu_pu_folded:-unmeasurable}]"
    [ "${vu_pu_tracked:-0}" = 1 ] || vu_bad="$vu_bad git-does-not-track-the-unfolded-spelling-so-there-is-no-mismatch"
    # PARTIAL, and this is the measurement that makes the arm about the new
    # property rather than the old one. If every shipped path were
    # unrecognisable the guard that only ever demanded a non-zero overlap would
    # have fired too, and this would prove nothing it did not already hold.
    [ "${vu_pu_ships:-0}" -gt "${vu_pu_folded:-0}" ] 2>/dev/null \
