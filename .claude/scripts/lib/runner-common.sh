...
# parent's stop began, because a process of the tree can start one between the
# list and the stop. A child started while its parent's stop ran is found only by
# the nonce, because after the stop the parent's id may belong to a process that
# has come and gone. A child whose parent id now belongs to a newer process,
# which started no later than the child, is that process's, and is left alone.
# An id already picked counts as picked only while it still belongs to the same
# process. Rounds end once nothing picked is left.
#
...
run_with_watchdog() {
  ...
  if [ -f "$flag.stopped" ]; then
    ...
    # Set before the growth check, so a signal handler during it sees the result.
    case "$RUN_KILL_REPORT" in *alive*|*unknown*) RUN_KILL_FAILED=1 ;; esac
...
write_tripwire() {
  ...
  [ -n "$body" ] || return 1
  # The state directory copy first. A signal handler takes that copy as proof
  # that this run set the tripwire, and adds to it instead of replacing it.
...
# case_renamed <old> <new> <snapshot-that-must-lack-the-other>
# True when <old> and <new> are one plain file under two names, neither a link,
# and the snapshot does not list the name it must lack: <new> when it is the
# snapshot from before the pass, <old> when it is the one from after. That
# absence is what a writer that replaced the file under a new case leaves, and
# a link or a second hard link would not.
case_renamed() {
  local lack="$2"
  case "$3" in *after) lack="$1" ;; esac
  [ ! -L "$root/$1" ] && [ ! -L "$root/$2" ] \
    && ! P="./$lack" awk '{ line = $0; sub(/^[^ ]* [^ ]* /, "", line) } line == ENVIRON["P"] { found = 1; exit } END { exit found ? 0 : 1 }' "$3" 2>/dev/null
}
...
revert_path() {
  ...
  if [ -n "$b_blob" ]; then
    ...
    if P="./$p" awk '{ line = $0; sub(/^[^ ]* [^ ]* /, "", line) } line == ENVIRON["P"] { found = 1; exit } END { exit found ? 0 : 1 }' "$snap/before" \
      ...
      rc=1
    elif twin="$(case_twin "$p" "$snap/before")" && [ -n "$twin" ] && [ "$root/$p" -ef "$root/$twin" ] && case_renamed "$twin" "$p" "$snap/after"; then
      ...
      if [ -z "$t_blob" ]; then
        printf '    %s (the same file as %s, whose name differs only in case and which is in no commit, so it was left as it is)\n' "$p" "$twin" >> "$log"
        rc=1
      elif [ "$(index_blob "$root" HEAD "$twin")" != "$t_blob" ]; then
        printf '    %s (the same file as %s, whose name differs only in case and which was committed while the pass ran, so it was left as it is)\n' "$p" "$twin" >> "$log"
        rc=1
...
# was there before the pass, and returns 0, or 1 when the commit before the pass
# could not be listed. A note was there when the commit HEAD pointed at before
# the pass (HEAD_BEFORE) holds it, or when the snapshot taken before the pass
# lists it and it is not a leftover of this runner that adopt_uncommitted took
# back. The snapshot test also catches a note git ignores and one git's lookup
# misses, such as after a case-only rename, so a lookup that finds nothing does
# not let a change through. Without a snapshot to read, a long-tier path counts
# as there. A path whose name differs only in ASCII case from another name that
# either test knows is that note on a filesystem that folds case, as every clone
# on Windows or macOS does, so it counts as there too, an adopted leftover
# included, and is printed saying so. Case that differs outside ASCII, and names
# that differ only in Unicode normalization, are not caught.
long_tier_existing() {
  local root="$1" snap="$3" p before folded="" listed=0
  before="${HEAD_BEFORE:-HEAD HEAD}"
  before="${before##* }"
  while IFS= read -r p; do
    case "$p" in 31-standards/*|40-llm-wiki/wiki/*) ;; *) continue ;; esac
    if [ "$listed" -eq 0 ]; then
      # Every long-tier name either test knows, in lower case and then as it
      # is, for the case test, which must find a name that differs, or it would
      # stand in for the two tests. Listed once, on the first long-tier path,
      # with pathspec magic off, and a listing that fails is not a short list.
      listed=1
      if [ "$before" != NONE ]; then
        folded="$( ( set -o pipefail; cd "$root" && MSYS_NO_PATHCONV=1 GIT_TERMINAL_PROMPT=0 GIT_LITERAL_PATHSPECS=0 \
                       GIT_GLOB_PATHSPECS=0 GIT_NOGLOB_PATHSPECS=0 GIT_ICASE_PATHSPECS=0 \
                       git ls-tree -r -z --name-only "$before" -- 31-standards 40-llm-wiki/wiki 2>/dev/null | tr '\0' '\n' ) )" \
          || return 1
      fi
      folded="$( { [ -z "$folded" ] || printf '%s\n' "$folded"
                   [ -f "$snap/before" ] && awk '{ sub(/^[^ ]* [^ ]* \.\//, ""); print }' "$snap/before"; } \
                 | LC_ALL=C awk '{ printf "%s\t%s\n", tolower($0), $0 }' )"
    fi
    if [ "$before" != NONE ] && [ -n "$(index_blob "$root" "$before" "$p")" ]; then
      ...
      continue
    elif [ ! -f "$snap/before" ]; then
      printf '    %s (there is no snapshot from before the pass to tell)\n' "$p"
    elif P="./$p" awk '{ line = $0; sub(/^[^ ]* [^ ]* /, "", line) } line == ENVIRON["P"] { found = 1; exit } END { exit found ? 0 : 1 }' "$snap/before"; then
      printf '    %s (in no commit under this name)\n' "$p"
    fi
  done < "$2"
  return 0
}
...
append_run_log() {
  ...
  # A runner stopped in the middle of an append leaves its temporary files. Their
  # names have a part of their own, so no other file beside the log matches.
  rm -f "$out".runner-tmp.?????? 2>/dev/null
  ...
  if ! tmp="$(mktemp "$out.runner-tmp.XXXXXX" 2>/dev/null)"; then
  ...
  if [ -n "$old" ] && ! cat "$old" > "$tmp" 2>/dev/null; then
    printf '[%s] WARNING: %s could not be copied beside it, for example on a full disk, so it was left as it was and the run output was not added to it.\n' "$(ts)" "$out" >> "$log"
    rm -f "$tmp"
    return 1
  fi
  ...
  if [ "$(file_size "$tmp")" -gt "$max" ]; then
    if part="$(mktemp "$out.runner-tmp.XXXXXX" 2>/dev/null)" && cap_copy "$tmp" "$part" "$max" && mv -f "$part" "$tmp" 2>/dev/null; then
      :
    else
...
report_stop() {
  local log="$1" what="$2" root="$3" state="$4" runner="$5" idx="" list="" lock="" backup="" writes=""
  idx="$(git_index_lock_path "$root")"
  if [ -n "$idx" ] && [ -e "$idx" ]; then
  ...
  if [ "${RUN_KILL_FAILED:-0}" -eq 1 ]; then
    ...
    # Before a runner has recorded that containment ran (its signal handler, or a
    # containment error), the wording leaves containment out.
    if [ "${CONTAINMENT_CHECKED:-1}" -eq 0 ]; then
      writes="$what may still be running, so what it writes is unchecked"
    else
      writes="$what may still be running, so what it writes after containment is unchecked"
    fi
    # The tripwire counts as this run's only when containment set it, or its copy
    # is in the state directory, where the pass cannot write. Otherwise a tripwire
    # found only in the vault was written by the pass, and is replaced, and so is
    # a tripwire of which no copy is a regular file.
    if { [ "${CONTAINED:-0}" -eq 1 ] || [ -f "$state/$(basename "$TRIPWIRE_REL")" ]; } \
       && note_tripwire "$root" "$state" "$log" "Also, a process of the stopped $writes. $lock $backup End it, then review the vault. What the stop found:
$(sed 's/^/  /' "$list")"; then
      :
    elif ! write_tripwire "$root" "$state" "$runner" "a process of the stopped $writes" \
...
# then writes a missing copy from the one that is there. Returns 1 when neither
# copy is a regular file, so the caller writes a whole tripwire instead.
note_tripwire() {
  local root="$1" state="$2" log="$3" text="$4" f="" found=1 vault="$1/$TRIPWIRE_REL" kept="$2/$(basename "$TRIPWIRE_REL")"
  for f in "$vault" "$kept"; do
    [ -f "$f" ] && [ ! -L "$f" ] || continue
    found=0
    printf '\n%s\n' "$text" >> "$f" 2>/dev/null \
  ...
  [ "$found" -eq 0 ] || return 1
  if [ ! -f "$kept" ] || [ -L "$kept" ]; then
    write_file_atomic "$kept" "$vault" \
      || printf '[%s] WARNING: the tripwire could not be copied to the state directory %s.\n' "$(ts)" "$state" >> "$log"
  elif [ ! -f "$vault" ] || [ -L "$vault" ]; then
    write_file_atomic "$vault" "$kept" \
      || printf '[%s] WARNING: the tripwire could not be copied to %s.\n' "$(ts)" "$vault" >> "$log"
  fi
  return 0
}
