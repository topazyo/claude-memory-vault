...
this template I researched that landscape with the goal of *not* building anything. I wanted to find the best
...
The requirements came from how I work, not from a feature wishlist. They are listed in
...
4. **A small, earned long tier.** The notes that steer future sessions should be few, verified and
   dated. Capture can be cheap and noisy, but promotion must be deliberate.
...
score. That pass was needed. One research agent had invented an alarming security finding about
...
omitted throughout, because the research found them unreliable in this category.
...
| --- | --- | --- |
| **mem0** | Hybrid vector/graph/key-value store. ADD, UPDATE, DELETE and NOOP as extraction outcomes, and expiry dates on writes | An LLM extraction pipeline writes memory with no human in the loop, which is the failure its own issue tracker documents best (see [section 4](#4-what-the-landscape-taught)) |
...
| **Letta (MemGPT)** | Memory blocks with hard character limits (so every write is an eviction decision), sleep-time consolidation outside the working session, and git-backed Context Repositories | A full agent runtime with Postgres and pgvector. Its own argument for git-backed markdown is one of the strongest endorsements of this vault's substrate |
| **Supermemory** | Temporary facts expire after their date passes, and contradictions are resolved automatically | Resolution *executes* rather than being proposed, the opposite default from requirement 2 |
...
| Project | Distinctive idea | Why it was not adopted here |
| --- | --- | --- |
| **[GBrain](https://github.com/garrytan/gbrain)** | The markdown repo is the system of record and the database a rebuildable cache, enforced by a CI check. Hybrid keyword, vector and graph retrieval with reranking. Cited synthesis that lists what it *doesn't* know, a large MCP tool surface, and a multi-user mode | As read in August 2026, a Bun runtime with macOS and Linux builds only, Ubuntu-only CI, no Task Scheduler path, and a Unix-socket lock. It also needs a database and a daemon, and its background cycle includes LLM phases that write into your notes |
...
| **eugeniughelbur/obsidian-second-brain** | Closest by architecture. A lesson state machine (active, stale, superseded, promotion candidate) promoting lessons seen three or more times. A vault health audit, an index staleness budget, and optional semantic search that is local and off by default | An AI-first vault whose agents rewrite notes, with a recall hook that overlapped tooling I already run. Its lesson lifecycle is the nearest thing to this vault's goal, already shipped |
| **Basic Memory** + **basic-memory-skills** | Markdown files with a SQLite index, edited by human and model alike, plus skills for between-session reflection, lifecycle and "defrag" | The defrag pass rewrites the corpus for clarity, which trades provenance for tidiness. AGPL-3.0. Its skills repo states a principle this vault shares, that memory forms better between sessions than during them |
| **breferrari/obsidian-mind** | Deterministic TypeScript hooks own the vault structure and validate writes, with hybrid retrieval | Adds a TypeScript toolchain and its own structure. The write-validation idea was reimplemented here as a bash lint hook |
| **heyitsnoah/claudesidian** | Clone-as-vault PARA layout with a "thinking mode" and grep-only retrieval | Clone-as-vault means its layout *is* your vault. Its grep-first stance matches this one |
...
| **lucasrosati/claude-code-memory-setup** | A documented recipe combining Zettelkasten notes with an AST-derived code graph | A recipe rather than a system, and the code-graph step depends on an unaudited third-party package |
...
missing discipline of freshness, verification and write safety.** That gap, not another engine, is
...
| --- | --- | --- |
| **claude-mem** | Hooks capture every session automatically into SQLite with full-text and vector search, injected at session start | It cannot forget to capture, but it captures indiscriminately. It complements this vault rather than replacing it, and the two run side by side well |
...
These solve a problem this vault does have, semantic search, without touching the write path.
...
bi-temporal invalidation is the principled answer, and this vault's `superseded_by` and `contradicts`
...
- **Consolidation is careful rather than automatic.** Supermemory and mem0 resolve and forget on
  their own, with the risks section 4 describes. This vault's consolidation only proposes, so it
  only helps if the scheduled passes run and someone acts on their proposals.
...
These choices are not mutually exclusive. The vault is plain markdown, so an indexer can read it, an
automatic capture tool can feed its inbox, and a retrieval plugin can search it. The part this
repository insists on is the promotion gate between cheap capture and trusted memory. That gate
works the same whatever sits on either side of it.
