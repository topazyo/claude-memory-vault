...
minutes each) to commit its journal, plus a margin. Then the runner always kills a hung pass first, logs
...
| Runner exits 2 and the log says the pass changed files that already had uncommitted changes | You, a sync client or another tool had uncommitted edits in a journal or long-tier note, and the pass wrote to the same file | Nothing was committed. Look at the file with `git diff`, keep what you want, commit it yourself, and the next pass runs normally |
| Dream runner exits 5 and the log says `CHECK-FAILED` | The journal the pass wrote fails `vault-check.sh` (C1–C5), so it was not committed | Read the check output under that line, fix the journal by hand or delete it, then commit or rerun |
| Dream runner exits 4 and the log says `COMMIT-FAILED` | Staging or committing the journal failed or ran past `RUNNER_GIT_TIMEOUT`: a failing commit hook (the opt-in gate checks every note), a signing key that needs a passphrase, no git identity, or a busy `index.lock` | The log shows git's output. Fix the cause, then commit the journal yourself or rerun. The journal is left in place and unstaged. Delete an `index.lock` the log names once no git command is running |
