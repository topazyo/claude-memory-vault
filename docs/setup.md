...
| Dream or promotion runner exits 2 and the log says `VIOLATION` | A file outside the pass's allowed folders changed during the run — the agent, or another writer such as a sync client. For `VIOLATION: long-tier notes that were there before the pass started changed during it`, see that row below | Read the paths listed under the `VIOLATION` line in `.claude/logs/`, and revert with git anything you did not expect. If the log also names notes under `The pass also changed long-tier notes that were there before it started`, those are still as the pass wrote them: restore each with `git restore -- <path>` |
| Promotion runner exits 2 and the log says `VIOLATION: long-tier notes that were there before the pass started changed during it` | The pass edited, stamped or retired a standard or wiki entity that was there before it, which it may only propose, or you or a sync client edited one while it ran | Nothing was committed by the runner, and the pass's notes were put back and listed under `REVERTED`. A changed note's bytes are copied to the quarantine the log names, where an edit you made during the pass can be recovered, unless the log lists the note as committed while the pass ran or changed after it ended, when it was left as it is. A sync client that committed during the pass may have committed the change itself: revert that commit, and pause auto-commit around the scheduled pass. If the agent did it, check that a customized `promotion-agent.md` no longer tells it to stamp, supersede or edit an existing note |
