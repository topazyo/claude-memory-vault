...
| Runner exits 2, the log says `contained`, and `.claude/logs/runner-tripwire` exists | The pass changed a steering or execution surface: an Obsidian plugin or config file, something under `.claude/`, a harness config, an instruction file, memory, or git's config, hooks or refs. The runner restored those files and moved what the pass wrote into the quarantine outside the vault | Read the tripwire, which names each path and the quarantine directory. Inspect the quarantined files, and check `git reflog` if a ref is listed. Delete the tripwire only once you know what happened |
| Runner exits 78 and the log says `TRIPWIRE`, or `vault-check.sh` says `TRIPWIRE` | A tripwire from an earlier contained violation is still in place, so nothing runs | As above: review, then delete `.claude/logs/runner-tripwire` |
